...
const analyzeText = (text) => {
    ...
    let projectsCount = 0;
    const projectMatch = text.match(/(\d+)\s*(?:\+\s*)?(?:projects?|apps?|applications?|systems?|tools?)/i) 
                      || text.match(/(?:built|created|developed|completed)\s*(\d+)/i);
    if (projectMatch) {
        projectsCount = parseInt(projectMatch[1], 10);
    } else {
        const anyNumber = text.match(/\b([1-9]|[1-4][0-9]|50)\b/);
        if (anyNumber) {
            projectsCount = parseInt(anyNumber[1], 10);
        }
    }
...
const fetchFromGemini = async (apiKey, bodyPayload) => {
    const models = ['gemini-2.5-flash', 'gemini-2.0-flash', 'gemini-1.5-flash', 'gemini-3.1-flash-lite-preview'];
